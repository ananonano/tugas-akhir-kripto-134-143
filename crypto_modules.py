...
def lsb_hide_message(image, secret_message):
    img = image.copy()
    binary_secret = data_to_binary(secret_message) + '1111111111111110'
        ...
        for x in range(image.width):
            ...
            for i in range(3):
                ...
                if binary_data.endswith(delimiter):
                    data_sebelum_delimiter = binary_data[:-len(delimiter)]
                    all_bytes = [data_sebelum_delimiter[j:j+8] for j in range(0, len(data_sebelum_delimiter), 8)]
                    decoded_message = ""
